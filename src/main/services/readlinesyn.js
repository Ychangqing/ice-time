...
var LineReader = function (path) {
    ...
    this._buffer = Buffer.alloc(this._bufferSize);
    ...
    if (undefined !== path) {
        try {
            ...
            console.log('[41;30m commitError', path, '[40;31m 无法找到文件！ \n')
            this._EOF = false;
            return;
        }
    }
...
LineReader.prototype.close = function () {
    ...
    try {
        ...
        console.log('[40;31m closing file failed')
    }
    self._EOF = true;
    self._fd = 0;
    return;
...
LineReader.prototype.next = function () {
    ...
    while ((self._leftOver.indexOf('\n', _idxStart)) == -1) {
        ...
        try {
            ...
            console.log('[40;31m reading file failed')
            self.close();
            return;
        }
...
LineReader.prototype.open = function (thePath) {
    ...
    try {
        ...
        console.log('[41;30m commitError', self._filename, '[40;31m 无法找到文件！ \n')
        self._EOF = true;
        return;
    }
